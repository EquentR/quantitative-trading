...
个人沪深 A 股和场内 ETF 短线量化决策辅助系统。系统只输出可解释建议，不会自动下单，不控制真实交易客户端，不绕过人工确认。
...
本地观察池是候选买入证券池来源，不代表真实持仓。系统可以读取东方财富自选作为只读候选，也可以按名称或代码搜索沪深 A 股和沪深场内 ETF；两种来源都必须先生成 10 分钟预览，再由用户多选确认，预览本身不会改变观察池或东方财富账户。

观察池记录 `symbol,name,rank,plan_enabled,note`，并关联后端验证的交易所、证券类型、T+0/T+1 制度和元数据来源。记录来源为：

- `manual`：本地手动维护。
- `synced`：用户从东方财富候选中确认加入。
- `manual_synced`：东方财富候选命中本地手动记录，保留本地 `plan_enabled` 和备注。

新确认的已验证 A 股和 ETF 默认 `plan_enabled=true`；已有记录保留当前开关和备注。交易制度未知的 ETF 可以加入展示，但强制 `plan_enabled=false`；证券类型未知的候选不可选择。非持仓观察证券只有 `plan_enabled=true` 时才会进入收盘计划和盘中展示采集，且仍须进入当日活动计划后才能参与盘中决策；持仓台账中的证券始终按持仓来源进入计划。
...
预览东方财富候选不会写观察池；显式传入代码或消费搜索预览后才会增量加入：

```bash
qt watchlist sync
qt watchlist sync --symbols 600519,510300
qt watchlist search 沪深300
qt watchlist select <preview-id> --symbols 510300
```

`qt watchlist sync` 无 `--symbols` 时只输出预览摘要。搜索不要求东方财富 Key，也不会调用自然语言选股接口。JSON/CSV 导入仍是全量替换当前观察池；导入前会尽力刷新证券目录，无法验证证券类型或交易制度的记录会保留展示但强制关闭计划，并返回 warning。Web 会明确提示全量替换语义并展示这些 warning。
...
重数据只覆盖手动持仓和 `plan_enabled=true` 的非持仓自选；其中非持仓观察证券还必须具备已验证元数据。盘中报价、分钟线和分时强弱覆盖当前持仓、活动计划和这些已启用观察标的，但账户估值、策略、风控、建议和通知仍只处理当前持仓与活动计划标的。重复标的只采集一次，provider 返回的额外证券不会扩大股票池。

系统为启用标的维护最近 250 个 `XSHG` 交易日的前复权日 K 和当前交易日 1 分钟行情。A 股另维护 60 个交易日的完整资金流；ETF 资金流保存为稳定的 `not_applicable`，不算 provider 失败、不降低总体质量，也不能提供资金确认。日 K 和适用的资金流使用追加版本及不可变快照；原始分钟线只保留最近 20 个交易日，但分时强弱、规则版本、建议输入和审计引用长期保留。

行情 adapter 按后端证券元数据把 A 股和 ETF 路由到各自的报价、前复权日 K 和分钟接口；东方财富分钟接口失败时在 adapter 内降级到 AkShare 新浪分钟接口，并保存实际 source。A 股固定 T+1；ETF 只根据交易所产品目录的明确类别映射 T+0/T+1，禁止按代码前缀或名称猜测。制度或涨跌停比例未知时只观察；持仓场景保守 `hold` 并要求人工复核。手动台账的 `available_quantity` 始终优先，即使是 T+0 ETF，数量为 0 时仍禁止 `sell/reduce`。
...
东方财富和妙想相关 API key、token、cookie、账号信息不得提交到 git。经用户确认，东方财富/妙想 Key 由 Web 或 `/api/v1/datasource/eastmoney/key` 明文保存在本地 SQLite，供后端 adapter 运行时读取；API、CLI、Web 回填、日志、审计、错误和测试快照永远不返回原值。`data/`、数据库和备份必须保持 git 忽略并限制为本机用户可读。`QT_DATABASE_PATH` 指向的数据库备份会包含该 Key。

“检查连接”会真实执行一次只读自选查询：成功空列表也算 configured；认证失败标为 invalid；额度耗尽、超时、网络或供应商契约变化保留原配置状态并记录脱敏错误。系统不会调用东方财富自选新增或删除接口。
...
SMTP 密码经用户确认后明文保存在本地 SQLite。读取 API、CLI、Web 回填、日志、审计、错误和 outbox 永远不得包含原值；密码输入留空保留已有值，替换和清除必须是明确操作。数据库导出和备份会包含 SMTP 密码及本地数据源 Key，因此 `data/`、备份和本地运行数据必须保持 git 忽略并限制为本机用户可读。

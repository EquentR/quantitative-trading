...
市场快照只采集手动持仓和 `plan_enabled=true` 的非持仓自选标的；关闭计划的非持仓自选会被排除，重复标的只采集一次。命令输出市场快照 ID、股票池快照 ID、请求及状态计数、数据时间和告警；全部失败或空股票池时 `data_time` 显示 `-`。

该命令只保存项目内部报价及失败记录，不输出第三方原始响应。它不会下单、控制交易客户端，也不读取或修改真实券商凭据或账户；市场快照尚未接入计划、策略、建议、通知或调度。

provider 整体失败或禁用行情抓取时，命令仍可成功退出，因为失败报价已持久化；调用方必须结合状态计数和 warnings 判断结果，不能只看退出码。数据库写入或配置校验失败仍会返回非零退出码。

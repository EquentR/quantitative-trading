...
服务未完成访问密码设置时允许启动，但业务接口不可用，并返回 `auth_status=setup_required` 或统一错误码 `auth_setup_required`。此阶段允许的最小公开访问面包括认证初始化、登录入口，以及只暴露认证启动状态的服务状态查询。
...
`GET /api/v1/service/status` 在未携带令牌时只返回认证 bootstrap 状态，不返回完整调度、路径或运行细节。完整服务状态需要 `Authorization: Bearer <access_token>`。
...
账户快照可读取手动持仓台账、手动资金账户和行情数据，并写入本地账户快照。行情缺失或覆盖不足时，快照必须显式保留状态，不得把不完整估值伪装成完整账户估值。

...
<template>
  <aside
    class="fixed inset-y-0 right-0 z-30 flex w-full max-w-sm flex-col gap-4 overflow-y-auto border-l border-border bg-background p-4 shadow-lg"
    aria-label="建议详情"
  ...
  >
    <header class="flex items-start justify-between gap-2">
      ...
      <Button variant="ghost" aria-label="关闭详情" @click="emit('close')">
        <X class="size-4" />
      </Button>
    </header>
    ...
    <template v-else>
      <section class="space-y-1">
        ...
        <ul class="list-disc space-y-0.5 pl-4 text-sm break-words">
